...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        if (App::environment('local', 'development')) {
            ...
            View::composer('*', function ($view) {
                $queries = DB::getQueryLog(); // Capture queries **after execution**
                $groupedQueries = [];
                ...
                foreach ($queries as $query) {
                    ...
                    if (!isset($groupedQueries[$formattedQuery])) {
                        $groupedQueries[$formattedQuery] = [
                            'query' => $formattedQuery,
                            'count' => 1,
                        ];
                    } else {
                        $groupedQueries[$formattedQuery]['count']++;
                    }
                }

                $view->with('groupedQueries', $groupedQueries);
                $view->with('queryCount', count($queries));
            });
        }
    }
    ...
    private function setConfigVars()
    {
        ...
        config()->set('services.googlemaps.key', env('', config('settings.other.googlemaps_key')));
        // Meta-tags
