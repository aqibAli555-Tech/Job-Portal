...
class MessageController extends AdminBaseController 
{
    ...
    public function index(Request $request)
    {
        ...
        $users = User::get_all_employer();
        return view('admin.message.index', compact('title', 'breadcrumbs', 'threads', 'users'));
    }
    ...
    public function show($id)
    {
        ...
        $linksRender = $messages->links('admin.message.message-chat.pagination')->render();
        ...
        $result = [];
        $result['messages'] = view('admin.message.message-chat.chat', ['messages' => $messages])->render();
    ...
    function actions($threadId = null)
    {
        ...
        if (request()->get('type') == 'delete') {
            $res = $this->deleteThreadByAdmin($threadId);
    ...
    {
        $result = Thread::getUnreadThreads();
        return response()->json($result, 200, [], JSON_UNESCAPED_UNICODE);
