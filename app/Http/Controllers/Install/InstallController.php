...
class InstallController extends Controller
{
    ...
    public function siteInfo(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            // Check purchase code
            $messages = [];
            // $purchaseCodeData = $this->purchaseCodeChecker($request->get('purchase_code'));
            ...
            if ($request->get('mail_driver') == 'sendmail') {
                $rules = array_merge($rules, $sendmail_rules);
            }
            if ($request->get('mail_driver') == 'smtp') {
                $rules = array_merge($rules, $smtp_rules);
            }
            if ($request->get('mail_driver') == 'mailgun') {
                $rules = array_merge($rules, $mailgun_rules);
            }
            if ($request->get('mail_driver') == 'postmark') {
                $rules = array_merge($rules, $postmark_rules);
            }
            if ($request->get('mail_driver') == 'ses') {
                $rules = array_merge($rules, $ses_rules);
            }
            if ($request->get('mail_driver') == 'mandrill') {
                $rules = array_merge($rules, $mandrill_rules);
            }
            if ($request->get('mail_driver') == 'sparkpost') {
            ...
            if ($request->get('mail_driver') == 'smtp') {
                ...
                $messages = [];
                try {
                    $transport = new Swift_SmtpTransport($request->get('smtp_hostname'), $request->get('smtp_port'), $request->get('smtp_encryption'));
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                ...
                if (empty($request->socket)) {
                    $dsn = $driver . ':host=' . $request->get('host') . ';port=' . $port . ';dbname=' . $request->get('database') . ';charset=utf8';
                } else {
                    $dsn = $driver . ':unix_socket=' . $request->get('socket') . ';dbname=' . $request->get('database') . ';charset=utf8';
                }

                // Connect to the Database Server
                $pdo = new PDO($dsn, $request->get('username'), $request->get('password'), $options);
