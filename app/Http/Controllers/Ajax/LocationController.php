...
class LocationController extends FrontController
{
    ...
    public function get_city_by_country($countryCode)
    {
        $cities = City::countryOf($countryCode)->orderBy('name')->get();
        ...
        echo json_encode(['items' => $citiesArr]);
        die;
//        return response()->json(['items' => $citiesArr], 200, [], JSON_UNESCAPED_UNICODE);
    }
    ...
    public function getAdmin1WithCities($countryCode, Request $request)
    {
        ...
        $currSearch = unserialize(base64_decode($request->get('currSearch')));
