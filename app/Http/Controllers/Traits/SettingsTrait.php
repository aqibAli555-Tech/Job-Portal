...
trait SettingsTrait
{
    ...
    public $cacheExpiration = 1; // In minutes (e.g. 60 * 60 for 1h)
...
//		setcookie('X-XSRF-TOKEN', csrf_token(), $this->cookieExpiration, '/', $domain);
        ...
        // Pages Menu
        $pages = Cache::remember('pages.' . config('app.locale') . '.menu', config('cache.stores.file.expire'), function () {
        ...
        // Get all Countries
        $this->countries = Cache::remember('countries_front_end', config('cache.stores.file.expire'), function () {
            // Your actual query here with filters
            return CountryLocalization::getCountries();
        });
