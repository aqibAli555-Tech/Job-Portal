...
class Country extends BaseModel
{
     ...
     * @var array
     */
    protected $fillable = ['code', 'iso3', 'iso_numeric', 'fips', 'name', 'asciiname', 'capital', 'area', 'population', 'continent_code', 'tld', 'currency_code', 'phone', 'postal_code_format', 'postal_code_regex', 'languages', 'neighbours', 'equivalent_fips_code', 'background_image', 'admin_type', 'admin_field_active', 'active'];
    ...
    */
    ...
    public static function get_all_country_with_employee_count()
    {
        return Country::
        select('countries.*', DB::raw('IFNULL(count(users.id), 0) as count'), DB::raw("JSON_UNQUOTE(JSON_EXTRACT(IF(JSON_VALID(countries.name), countries.name, JSON_OBJECT('en', countries.name)), '$.en')) as country_name"))
            ->leftJoin('users', function ($join) {
                $join->on('countries.code', '=', 'users.country_code')
    ...
    public static function get_all_country_employee_count($type)
    {
        return self::select('countries.*', \DB::raw('IFNULL(count(users.id),Null) as count'))->join('users', 'countries.code', '=', 'users.country_code', 'left')->where('users.user_type_id', $type)->orderBy('count', 'desc')->orderBy('countries.name', 'asc')->groupBy('countries.name')->groupBy('countries.code')->get();
    }

    public static function country_name_by_login_user_country_code()
    {
        $country = self::select('name')->where('code', auth()->user()->country_code)->first();
        $countryName = '';
        if (!empty($country->name)) {
            $decodeCountry = json_decode($country->name);
            if (!empty($decodeCountry->en)) {
                $countryName = $decodeCountry->en;
            } else {
    ...
    public static function get_all_country_with_postCount()
    {
        $country = Cache::remember('get_all_countries_with_post_count', config('cache.stores.file.expire'), function () {
            // Your actual query here
            return self::select('countries.*', DB::raw('IFNULL(count(posts.id),0) as count'))
                ->leftJoin('posts', function ($join) {
                    $join->on('countries.code', '=', 'posts.country_code')
    ...
    public static function get_country_by_code($id)
    {

        return Country::where('code', $id)->first();
    }
