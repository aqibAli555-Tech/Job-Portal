...
class Category extends BaseModel
{
     ...
     * @var array
     */
    protected $fillable = ['translation_lang', 'translation_of', 'parent_id', 'name', 'slug', 'description', 'picture', 'icon_class', 'lft', 'rgt', 'depth', 'type', 'active'];
    /**
    ...
    */
    ...
    public static function selectBoxTree($skippedId = null, $entries = [], &$tab = [], $level = 0, $spacerChars = '-----')
    {
        ...
        if (empty($entries)) {
            ...
            $entries = EmployeeSkill::getAllskill();
...
//			}
            ...
            if ($skippedId != $entry->id) {
                $tab[$entry->id] = $spacer . $entry->skill;
    ...
    public static function countPostsByCategory($cityId = null)
    {
        ...
        $sql = 'SELECT parent.id, COUNT(*) AS total
				...
				WHERE node.lft BETWEEN parent.lft AND parent.rgt
					    ...
					    AND tPost.archived != 1 AND tPost.deleted_at IS NULL
				GROUP BY parent.id';
        $bindings = [
