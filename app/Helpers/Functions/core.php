...
use App\Helpers\Ip;
use App\Helpers\Localization\Country;
...
{
    return Ip::get($canGetLocalIp, $defaultIp);
}
 ...
 * @param null $locale
 * @return false|UrlGenerator|null|string
 */
 ...
 *
 * @param string|Collection $country
 * @param string $path
 * @param bool $forceCountry
 * @param bool $forceLocale
 * @return UrlGenerator|string
...
function getValidCountry($country)
{
    ...
    if (is_string($country)) {
        if (strlen($country) == 2) {
            $country = Country::getCountryInfo($country);
...
function getCountryMainLangCode($country)
{
    ...
    if ($country->has('lang') && $country->get('lang')->has('abbr')) {
        ...
        if ($country->has('languages')) {
            $countryLang = Country::getLangFromCountry($country->get('languages'));
...
function isFromApi()
{
    ...
    if (
        ...
        && Str::contains(\Route::currentRouteAction(), plugin_namespace('apijc'))
    ) {
...
function isDemo($url = null)
{
    if (isDemoDomain()) {
        if (
            Str::contains(\Route::currentRouteAction(), 'Account\MessagesController@store')
        ...
        if (auth()->check()) {
            if (isFromAdminPanel($url)) {
                ...
                if (
                    ...
                    && !Str::contains(\Route::currentRouteAction(), 'Account\MessagesController')
                    && !Str::contains(\Route::currentRouteAction(), 'Account\ResumeController')
                    && !Str::contains(\Route::currentRouteAction(), 'Auth\LoginController')
                    && !Str::contains(\Route::currentRouteAction(), 'Locale\SetLocaleController')
                    && !Str::contains(\Route::currentRouteAction(), 'reviews\app\Http\Controllers\ReviewController')
                ) {
                    return true;
                }
            }
...
function readableBytes($bytes, $decimals = 2, $system = 'binary')
{
    ...
    if (is_numeric($bytes)) {
        ...
        $bytes = Number::format($bytes, $decimals);
        $bytes = $bytes . $unit;
    }
...
function isAvailableLang($abbr)
{
    ...
    $lang = \Cache::remember('language.' . $abbr, $cacheExpiration, function () use ($abbr) {
        $lang = Language::where('abbr', $abbr)->first();
...
function jsonToArray($string)
{
    ...
    if (is_object($string)) {
        return ArrayHelper::fromObject($string);
    }
 ...
 * @param int $format
 * @return PhoneNumberUtil|mixed|string
 */
function phoneFormat($phone, $countryCode = null, $format = PhoneNumberFormat::NATIONAL)
{
    // Country Exception
    if ($countryCode == 'UK') {
        $countryCode = 'GB';
 ...
 * @return PhoneNumberUtil|mixed|string
 */
function phoneFormatInt($phone, $countryCode = null, $format = PhoneNumberFormat::INTERNATIONAL)
{
...
function getPhoneIcon($countryCode)
{
    if (file_exists(public_path() . '/public/images/flags/16/' . strtolower($countryCode) . '.png')) {
        $phoneIcon = '<img src="' . url()->asset('images/flags/16/' . strtolower($countryCode) . '.png') . getPictureVersion() . '" style="padding: 2px;">';
    } else {
...
function getMetaTag($tag, $page)
{
    ...
    if (config('plugins.domainmapping.installed')) {
        $out = Domainmapping::getMetaTag($tag, $page);
        if (!empty($out)) {
            return $out;
        }
    }
 ...
 *
 * @param null $skin
 * @return Repository|mixed|null|string
 */
...
function getSitemapsIndexes($htmlFormat = false)
{
    ...
    $countries = \App\Helpers\Localization\Helpers\Country::transAll(Country::getCountries());
    if (!$countries->isEmpty()) {
        ...
        foreach ($countries as $country) {
            $country = Country::getCountryInfo($country->get('code'));
...
{
    try {
        if (!DotenvEditor::keyExists('FALLBACK_LOCALE_FOR_DB')) {
            DotenvEditor::addEmpty();
        }
        DotenvEditor::setKey('FALLBACK_LOCALE_FOR_DB', $fallbackLocale);
        DotenvEditor::save();
    } catch (Exception $e) {
    }
}
...
function get_company_logo($path)
{
    if (file_exists($path)) {
        return storage_path($path);
    } else {
        return url()->asset('images/logocv.png');
    }
}
