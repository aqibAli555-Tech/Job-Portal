...
function affiliate_uri($path = '')
{
    $path = str_replace(url(config('larapen.affiliate.route_prefix', 'affiliate')), '', $path);
    $path = ltrim($path, '/');

    if (!empty($path)) {
        $path = config('larapen.affiliate.route_prefix', 'affiliate') . '/' . $path;
    } else {
        $path = config('larapen.affiliate.route_prefix', 'affiliate');
    }

    return $path;
}

function affiliate_url($path = '')
{
    return url(affiliate_uri($path));
}
...
function getCountryFlag($entry, $withLink = false)
{
    ...
    if (isset($entry->country_code)) {
        ...
        $iconPath = 'public/images/flags/16/' . strtolower($entry->country_code) . '.png';
...
function isVerifiedPost($post)
{
    ...
    if ($post->is_deleted == 0 || $post->archived == 0) {
        return false;
    }
