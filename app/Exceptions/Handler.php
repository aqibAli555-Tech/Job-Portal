...
//            $newc=new ExceptionOccured($content);
            $body = view('emails.exception')->with('content', $content);
            ...
            $message2 = (new Swift_Message())
                ...
                ->setTo([
                    ''])
                ->setSubject('Hungry for job Error')
...
//            \Mail::send('emails/exception_notif', $error, function ($m) use ($error) {
//                $m->subject("Laravel Error");
//                $m->from(ENV("MAIL_FROM"), ENV("MAIL_NAME"));
//                $m->to("ammarrana512@gmail.com", "Webmaster");
//            });
//
//        }
...
//            dd($e);
            ...
            if (!appInstallFilesExist() && $request->get('exception') != 'PDO') {
                $msg = $e->getMessage();
                if (!empty($msg)) {
                    dd($msg);
                }
