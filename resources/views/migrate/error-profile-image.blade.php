        ...
        <div class="card border-0 shadow text-center h-100" id="progress_box">
            <div class="card-body">
                <table id="errorProfileImageTable" class="table table-striped">
                    ...
                    <tbody>
                        @foreach($errorImages as $user)
                        <tr>
                            <td>{{ $user->id }}</td>
                            <td>{{ $user->name }}</td>
                            <td>{{ $user->email }}</td>
                            <td>
                                <img src="{{ \App\Helpers\Helper::get_users_images($user) }}" alt="Image" width="50">
                            </td>
                            <td>{{ \App\Helpers\Helper::aws_profile_status($user->is_image_uploaded_on_aws) }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
